...
* You need to be comfortabel to work on a non WYSIWYG style, with no previews and live rendering.
...
* Line - creates a straight line of a specified lenght
* Label - creates a simple label that contains text in a single line
* Rectangle - creates a rectangular box of specficied width and height (todo)
...
* _units_ - it can be either "mm" (for millimeters) or "in" (for inches). This property sets how the units are interpreted throught the *whole* file. Wherever a diameter or length or width or height is specified, that number is assumed to be in this unit of measurement.
...
* _padding_ - this is the distance of the border from the panel edges. itdoes juat that and it does not move your design from the origin (0,0). It is purely for aesthetic reasons if you want the border to not touch the edge of the panel.
...
##### Circle
This is a cicle of a specific diameter with an outline and an optional drill mark (cross-hair). 
...
* _drillmark_ - specifies whether a cross-hair mark shoul dbe drawn inside the circle to aid as a drill point. It can be either _true_ or _false_ and if not specified it default to _false_ (no drill-mark)
...
* _baseline_ - specifies whether a solid circural line will be drawn following the baseline of the dial (that will be exctly where the knob ends).It can be either _true_ or _false_ and if not specified it default to _false_ (no line).
* _line_ - despite its name, this represents the width of the line used for draing the baseline
* _anchor_ - this is the point of the circle where the _middle_ of the unoccupied dial segment should rest. This a clock-wise angle ranging from 0 to 360 nd the reference point is the top (12'clock). This helps to rotate the dial around the center and center the range of the dial opposite to it.
* _extend_ - this is the range of the dial (in degrees) and depends on the actual control you are using. For example. a typical single turn potentiomenter has a range of 270 degrees and a typical variable capacitor a range of 180 degrees.
...
##### Scale
This is part of the dial and cannot appear by itself , it has to be inside a dial control. The description of this control allows all the common scales to be created, from linear scales that devive equally a specific range, to custom markers in arbitrary points around the dial.
...
* _ticks_ - specifies the ticks (marks) of the scale. Note that this is a collection, as multiple ticks can be overlayed to create divisions and subdivisions of the scale (see next example).
...
Since ticks is a collection, it means that we can add a secondary scale to show subsdivisions of the main scale. This is done by simply adding a second group with the minimal info. The example below extends the previous one by adding a scale with a step of 0.2 which divides the main scale in 5 equal marks, using shorter marks.
...
* _test_ - a list (comma separated values) of text to place over every mark.Can be an empty string (i.e "") if a marker isnot required to have text, but cannot be omitted.  
